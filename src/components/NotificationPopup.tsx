    ...
    <>
      ...
      <div className="fixed right-4 top-20 w-full max-w-md bg-white rounded-2xl shadow-2xl z-50 max-h-[calc(100vh-100px)] flex flex-col md:right-6">
        ...
        <div className="p-4 border-b border-[#E5E7EB] flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-1 hover:bg-[#F6F7F9] rounded-lg">
        ...
        <div className="p-4 border-b border-[#E5E7EB] flex gap-2">
          <button className="flex-1 px-3 py-2 text-sm border border-[#E5E7EB] rounded-lg hover:bg-[#F6F7F9] flex items-center justify-center gap-2">
          ...
          {notifications.length === 0 ? (
            ...
            <div className="divide-y divide-[#E5E7EB]">
              {notifications.map((notif) => (
                <div
                  key={notif.id}
                  className={`p-4 hover:bg-[#F6F7F9] cursor-pointer transition-colors ${
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <h4 className="text-[#0F172A] truncate">{notif.title}</h4>
                        <Clock className="w-4 h-4 text-[#94A3B8] flex-shrink-0" />
                      </div>
                      <p className="text-[#64748B] text-sm line-clamp-2 mb-2">
                        {notif.message}
                      </p>
                      <span className="text-[#94A3B8] text-xs">{notif.time}</span>
                    </div>
                  </div>
                </div>
              ))}
