import RaporTableRow from "./RaporTableRow";
import EmptyState from "./EmptyState";

export default function RaporTable({ 
  data, raporData, activeSemesterKey, onPrintItem, 
  ...
  const handleSelectAll = () => {
    if (isAllSelected) {
      ...
      setSelectedStudents(data.map(item => item.id)); 
    }
  };
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-slate-500 uppercase text-[11px] border-b border-slate-200 tracking-wider">
            <tr>
              {/* Kolom Checkbox Header (Kondisional) */}
              {isSelectionMode && (
              ...
              <th className="px-6 py-4 text-center">Ketercapaian (BSH/BSB)</th>
              <th className="px-6 py-4 text-center">Absen (S/I/A)</th>
              <th className="px-6 py-4 text-center">Status Berkas</th>
              <th className="px-6 py-4 text-right">Cetak Rapor</th>
            </tr>
          </thead>
            ...
            {data.length > 0 ? (
              data.map((item) => (
                <RaporTableRow 
                  ...
                  isSelectionMode={isSelectionMode} // Teruskan ke Row
                />
              ))
            ) : (
              <EmptyState colSpan={isSelectionMode ? 6 : 5} />
            )}
          </tbody>
        </table>
      </div>
