import BukuPenghubungTableRow from "./BukuPenghubungTableRow";

export default function BukuPenghubungTable({ 
  data, getPenghubungData, onPrintItem, 
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
            ...
            <tr>
              {isSelectionMode && (
                 <th className="px-6 py-4 text-left w-12 animate-in fade-in slide-in-from-left-2 duration-300">
                   <input type="checkbox" checked={isAllSelected} onChange={handleSelectAll} className="w-4 h-4 rounded border-slate-300 text-[#e94640] focus:ring-[#e94640] cursor-pointer" />
                 </th>
              )}
              <th className="px-6 py-4 text-left">Identitas Siswa</th>
              <th className="px-6 py-4 text-center">Rekap Harian</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {data.length > 0 ? (
              data.map((item) => (
                <BukuPenghubungTableRow 
                  key={item.id} item={item} 
                  penghubungData={getPenghubungData(item.id)} 
                  onPrint={onPrintItem}
