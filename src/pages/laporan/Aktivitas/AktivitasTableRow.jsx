...
export default function AktivitasTableRow({ 
  item, aktivitasData, onPrint,
  selectedStudents, setSelectedStudents,
  isSelectionMode 
}) {
  const isSelected = selectedStudents.includes(item.id);
  
  // Simulasi mengecek jumlah aktivitas bulan tersebut
  const jumlahAktivitas = aktivitasData?.length || 4;
  const statusKegiatan = jumlahAktivitas > 0 ? "Terekam" : "Kosong";
    ...
    <tr className={`hover:bg-slate-50 transition-colors ${isSelected && isSelectionMode ? 'bg-red-50/30' : ''}`}>
      ...
      <td className="px-6 py-4 text-center">
         <span className="font-bold text-slate-700 text-sm">
            {jumlahAktivitas} <span className="text-slate-400 font-medium text-xs">Kegiatan</span>
         </span>
      </td>

      <td className="px-6 py-4 text-center">
         <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${statusKegiatan === "Terekam" ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-slate-100 text-slate-500 border border-slate-200"}`}>
            {statusKegiatan}
         </span>
      </td>
